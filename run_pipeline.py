...
from src.metrics import metrics_table, pairwise_dm_tests
...
def run_eda(df: pd.DataFrame, model_df: pd.DataFrame) -> None:
    ...
    sns.scatterplot(
        ...
        x="total_attendances",
        y=TARGET,
        hue="crs_period" if "crs_period" in df else None,
        ax=ax,
    )
...
def run_sensitivity(model_df: pd.DataFrame) -> None:
    """Re-run ARIMA and Random Forest excluding CRS months."""
